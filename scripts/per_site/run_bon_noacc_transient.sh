#!/bin/bash
# Bonanza Creek (BNF-Bon) noACC Nfixation transient-only experiment (Exp 4):
...
#
# USAGE:
#   bash run_bon_noacc_transient.sh <NOFUN_CASEID>
set -e
...
CASEID=noacc_transient_bon_$(date +%Y%m%d)
SITE=BNF-Bon
...
cd ${AD_CASE}
./case.build 2>&1 | tee build_noacc_transient_bon.log
EXEROOT=${RUNROOT}/${CASEID}_${SITE}_I1850CNRDCTCBC_ad_spinup/bld
if [ ! -f "${EXEROOT}/e3sm.exe" ]; then
  echo "ERROR: Build FAILED. Check ${AD_CASE}/build_noacc_transient_bon.log"
  exit 1
fi
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "=== TRANSIENT JOB SUBMITTED (Bon Exp 4) ==="
echo "CASEID:         ${CASEID}"
echo "Job ID:         ${JID}"
echo "SourceMods:     noACC_fixed_funp_nfix (Bytnerowicz, temperate/boreal params)"
