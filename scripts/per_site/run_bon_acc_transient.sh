#!/bin/bash
# Bonanza Creek (BNF-Bon) ACC Nfixation transient-only experiment (Exp 5):
#   Reuses nofun_baseline AD+FN spinup (600yr, FUN/FUNP OFF).
#   Builds new exe with ACC_fixed_funp_nfix SourceMods.
#   Runs ONLY the transient (1851-2100) with FUN+FUNP ON + ACC Nfixation.
#
...
#       tc_soila10 < 18.5 C:  Tmin = -2.04, Topt = 32.10   (temperate/boreal base)
#       18.5 <= tc < 28.5 C:  Tmin = 0.697*tc - 14.93, Topt = 0.047*tc + 31.24
#       tc_soila10 >= 28.5 C: Tmin = 4.93,  Topt = 32.58
#     Tmax_fix = 43.98 (fixed)
#   - fun_cost_fix_Bytnerowicz_Acc() is active
#   - fpg_p = 1.0 guard when use_funp=TRUE
#
# Soil: US-Bon boreal CNP_parameters passed via --mod_parm_file_P
#
# USAGE:
#   bash run_bon_acc_transient.sh <NOFUN_CASEID>
set -e
...
CNP_FILE=/home/braghiere/BNF_tom/OLMT_BNF/define_site_files/US-Bon/CNP_parameters.nc
CLM1PT_DIR=/home/braghiere/BNF_tom/inputdata/BNFMIP_forcing_from_OCN/BNF_Bon/CLM1PT_data
CASEID=acc_transient_bon_$(date +%Y%m%d)
SITE=BNF-Bon
SOURCEMODS=${CASEROOT}/source_codes/ACC_temperate_funp_nfix
...
cd ${AD_CASE}
./case.build 2>&1 | tee build_acc_transient_bon.log
EXEROOT=${RUNROOT}/${CASEID}_${SITE}_I1850CNRDCTCBC_ad_spinup/bld
if [ ! -f "${EXEROOT}/e3sm.exe" ]; then
  echo "ERROR: Build FAILED. Check ${AD_CASE}/build_acc_transient_bon.log"
  exit 1
fi
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "=== TRANSIENT JOB SUBMITTED (Bon Exp 5) ==="
echo "CASEID:         ${CASEID}"
echo "Job ID:         ${JID}"
echo "SourceMods:     ACC_fixed_funp_nfix (Bytnerowicz, temperate/boreal ACC params)"
echo "Param file:     ${CLM_PARAMFILE}"
echo "CNP file:       ${CNP_FILE}"
echo "Spinup reused:  ${NOFUN_CASEID}  (nofun_baseline FN spinup yr 751)"
echo "Nfixation:      Bytnerowicz ACC  Tmin/Topt shift with tc_soila10"
