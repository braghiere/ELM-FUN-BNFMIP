#!/bin/bash
# Bonanza Creek (BNF-Bon) FUN+FUNP transient-only experiment (Exp 3):
#   Reuses nofun_baseline AD+FN spinup (600yr, FUN/FUNP OFF).
#   Builds new exe with fun_fpg1_nfix SourceMods.
#   Runs ONLY the transient (1851-2100) with FUN+FUNP ON.
#
# SourceMods: fun_fpg1_nfix
#   - fpg(c)   = 1.0 when use_fun=TRUE  (diagnostic fix)
#   - fpg_p(c) = 1.0 when use_funp=TRUE (diagnostic fix)
#   - Houlton s_fix (from paramfile, not modified here)
#
# Soil: US-Bon boreal CNP_parameters passed via --mod_parm_file_P
#
# USAGE:
#   bash run_bon_fun_transient_only.sh <NOFUN_CASEID>
#   where NOFUN_CASEID is the CASEID used by submit_all_bon_experiments.sh Exp 1
set -e
...
CASEID=fun_transient_only_bon_$(date +%Y%m%d)
SITE=BNF-Bon
...
cd ${AD_CASE}
./case.build 2>&1 | tee build_fun_transient_bon.log
EXEROOT=${RUNROOT}/${CASEID}_${SITE}_I1850CNRDCTCBC_ad_spinup/bld
if [ ! -f "${EXEROOT}/e3sm.exe" ]; then
  echo "ERROR: Build FAILED. Check ${AD_CASE}/build_fun_transient_bon.log"
  exit 1
fi
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "=== TRANSIENT JOB SUBMITTED (Bon Exp 3) ==="
echo "CASEID:         ${CASEID}"
echo "Job ID:         ${JID}"
echo "SourceMods:     fun_fpg1_nfix (fpg=fpg_p=1.0)"
echo "Param file:     ${CLM_PARAMFILE}"
echo "CNP file:       ${CNP_FILE}"
echo "Spinup reused:  ${NOFUN_CASEID}  (nofun_baseline FN spinup yr 751)"
echo "Nfixation:      Standard Houlton (no acclimation override)"
