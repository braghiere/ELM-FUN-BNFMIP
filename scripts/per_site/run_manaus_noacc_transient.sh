#!/bin/bash
# Manaus noACC Nfixation transient-only experiment (Exp 4):
#   Reuses nofun_baseline AD+FN spinup (600yr, FUN/FUNP OFF).
#   Builds new exe with noACC_fixed_funp_nfix SourceMods.
#   Runs ONLY the transient (1850-2101) with FUN+FUNP ON + noACC Nfixation.
#
# SourceMods: noACC_fixed_funp_nfix
#   - Bytnerowicz Nfixation temperature response WITHOUT acclimation:
#     Tmin_fix, Topt_fix, Tmax_fix are fixed constants (no dependence on tc_soila10)
#       Tmin_fix = -2.04 C, Topt_fix = 32.10 C, Tmax_fix = 43.98 C
#   - fun_cost_fix_Bytnerowicz_noAcc() is active (noAcc version)
#   - fpg_p = 1.0 guard when use_funp=TRUE
#
# USAGE:
#   bash run_manaus_noacc_transient.sh <NOFUN_CASEID>
#   where NOFUN_CASEID is the CASEID used by run_manaus_nofun_baseline.sh
#   e.g.: bash run_manaus_noacc_transient.sh nofun_baseline_manaus_20241210
#
# Run on a compute node:
#   srun -A ccsi -p burst -N 1 -n 1 -t 4:00:00 --mem 32G \
#        --exclude=or-condo-c105,or-condo-c67 --pty bash
#   cd /home/braghiere/BNF_tom/OLMT_BNF && bash run_manaus_noacc_transient.sh <NOFUN_CASEID>
set -e
...
CASEID=noacc_transient_manaus_$(date +%Y%m%d)
SITE=BNF-Man
SOURCEMODS=${CASEROOT}/source_codes/noACC_fixed_funp_nfix
...
#   noACC_fixed_funp_nfix mods are safe during AD (FUN/FUNP flags are off).
# ══════════════════════════════════════════════════════════════════════
echo ""
...
cd ${AD_CASE}
./case.build 2>&1 | tee build_noacc_transient.log
EXEROOT=${RUNROOT}/${CASEID}_${SITE}_I1850CNRDCTCBC_ad_spinup/bld
if [ ! -f "${EXEROOT}/e3sm.exe" ]; then
  echo "ERROR: Build FAILED. Check ${AD_CASE}/build_noacc_transient.log"
  exit 1
fi
...
echo "CASEID:         ${CASEID}"
echo "Job ID:         ${JID}"
echo "SourceMods:     noACC_fixed_funp_nfix"
echo "  Nfixation:    Bytnerowicz noACC — fixed Tmin/Topt/Tmax (no temperature acclimation)"
echo "  Tmin_fix =  -2.04 C, Topt_fix = 32.10 C, Tmax_fix = 43.98 C (constants)"
