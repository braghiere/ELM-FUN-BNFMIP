#!/bin/bash
LOG=/home/braghiere/rerun_ha1.log; exec >>"$LOG" 2>&1
echo "==== Ha1 re-run (PFT7 surfdata) $(date) ===="
...
SF=$IN/surfdata_BNF-Ha1_PFT7.nc; DOM=$IN/domain_BNF-Har_1x1.nc
...
for exp in nofun_baseline fun_transient_only noacc_transient acc_transient; do
  CID=${exp}_ha1_repro20260708
  echo "--- $exp $(date +%H:%M) ---"
  rm -rf $CD/${CID}_BNF-Ha1_I1850*_ad_spinup $RR/${CID}_BNF-Ha1_I1850*_ad_spinup 2>/dev/null
  cd $OLMT
  timeout 400 python3 runcase.py --site BNF-Ha1 --sitegroup BNF --machine cades --caseidprefix $CID \
    --model_root /home/braghiere/BNF_tom/E3SM_global_silent --ccsm_input $IN --runroot $RR --caseroot $CD --nofire \
    --clm_paramfile /home/braghiere/BNF_tom/clm_params_fun3_sfix01.nc \
    --clm1pt_dir $IN/BNFMIP_forcing_from_OCN/BNF_Har/CLM1PT_data --surffile $SF --domainfile $DOM --nopointdata \
  ...
  AD=$(ls -d $CD/${CID}_BNF-Ha1_I1850*_ad_spinup 2>/dev/null|head -1)
  [ -z "$AD" ] && { echo "  CREATE FAILED"; continue; }
  ...
  [ -f "$(./xmlquery EXEROOT --value)/e3sm.exe" ] || { echo "  BUILD FAILED"; continue; }
  jid=$(./case.submit --batch-args="--exclude=$EXC" 2>&1|grep -oE "job id is [0-9]+"|grep -oE "[0-9]+$")
  echo "  LAUNCHED -> ${jid:-FAIL}"
done
echo "HA1_RERUN_DONE $(date)" > /home/braghiere/rerun_ha1.DONE
