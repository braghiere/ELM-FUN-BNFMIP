#!/bin/bash
# Harvard Forest (BNF-Ha1) FUN+FUNP transient-only experiment (Exp 3):
...
#
# USAGE:
#   bash run_ha1_fun_transient_only.sh <NOFUN_CASEID>
#   where NOFUN_CASEID is the CASEID used by submit_all_ha1_experiments.sh Exp 1
set -e
...
CASEID=fun_transient_only_ha1_$(date +%Y%m%d)
SITE=BNF-Ha1
...
cd ${AD_CASE}
./case.build 2>&1 | tee build_fun_transient_ha1.log
EXEROOT=${RUNROOT}/${CASEID}_${SITE}_I1850CNRDCTCBC_ad_spinup/bld
if [ ! -f "${EXEROOT}/e3sm.exe" ]; then
  echo "ERROR: Build FAILED. Check ${AD_CASE}/build_fun_transient_ha1.log"
  exit 1
fi
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "=== TRANSIENT JOB SUBMITTED (Ha1 Exp 3) ==="
echo "CASEID:         ${CASEID}"
echo "Job ID:         ${JID}"
