#!/bin/bash
# Harvard Forest (BNF-Ha1) ACC Nfixation transient-only experiment (Exp 5):
...
#       tc_soila10 < 18.5 C:  Tmin = -2.04, Topt = 32.10   (temperate base)
#       18.5 <= tc < 28.5 C:  Tmin = 0.697*tc - 14.93, Topt = 0.047*tc + 31.24
#       tc_soila10 >= 28.5 C: Tmin = 4.93,  Topt = 32.58
#     Tmax_fix = 43.98 (fixed; small range so approximately fixed)
...
#
# USAGE:
#   bash run_ha1_acc_transient.sh <NOFUN_CASEID>
set -e
...
CASEID=acc_transient_ha1_$(date +%Y%m%d)
SITE=BNF-Ha1
...
cd ${AD_CASE}
./case.build 2>&1 | tee build_acc_transient_ha1.log
EXEROOT=${RUNROOT}/${CASEID}_${SITE}_I1850CNRDCTCBC_ad_spinup/bld
if [ ! -f "${EXEROOT}/e3sm.exe" ]; then
  echo "ERROR: Build FAILED. Check ${AD_CASE}/build_acc_transient_ha1.log"
  exit 1
fi
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "=== TRANSIENT JOB SUBMITTED (Ha1 Exp 5) ==="
echo "CASEID:         ${CASEID}"
echo "Job ID:         ${JID}"
echo "SourceMods:     ACC_fixed_funp_nfix (Bytnerowicz, temperate ACC params)"
