#!/bin/bash
# Manaus ACC Nfixation transient-only experiment (Exp 5):
#   Reuses nofun_baseline AD+FN spinup (600yr, FUN/FUNP OFF).
#   Builds new exe with ACC_fixed_funp_nfix SourceMods.
#   Runs ONLY the transient (1850-2101) with FUN+FUNP ON + ACC Nfixation.
#
# SourceMods: ACC_fixed_funp_nfix
#   - Bytnerowicz Nfixation temperature response WITH acclimation:
#     Tmin_fix and Topt_fix shift with 10-day mean soil temp (tc_soila10)
#       tc_soila10 < 18.5 C:  Tmin = 2.37, Topt = 30.34
#       18.5 <= tc < 28.5 C:  Tmin = 0.932*tc - 14.87, Topt = 0.574*tc + 19.72
#       tc_soila10 >= 28.5 C: Tmin = 11.69, Topt = 36.08
#   - fun_cost_fix_Bytnerowicz_Acc() is active (Acc version)
#   - fpg_p = 1.0 guard when use_funp=TRUE
#
# USAGE:
#   bash run_manaus_acc_transient.sh <NOFUN_CASEID>
#   where NOFUN_CASEID is the CASEID used by run_manaus_nofun_baseline.sh
#   e.g.: bash run_manaus_acc_transient.sh nofun_baseline_manaus_20241210
#
# Run on a compute node:
#   srun -A ccsi -p burst -N 1 -n 1 -t 4:00:00 --mem 32G \
#        --exclude=or-condo-c105,or-condo-c67 --pty bash
#   cd /home/braghiere/BNF_tom/OLMT_BNF && bash run_manaus_acc_transient.sh <NOFUN_CASEID>
set -e
...
CASEID=acc_transient_manaus_$(date +%Y%m%d)
SITE=BNF-Man
SOURCEMODS=${CASEROOT}/source_codes/ACC_fixed_funp_nfix
...
#   Only the transient will be submitted. AD case is used for building exe only.
...
#   ACC_fixed_funp_nfix mods are safe during AD (FUN/FUNP flags are off).
# ══════════════════════════════════════════════════════════════════════
echo ""
echo "=== Step 2: Copy SourceMods (ACC_fixed_funp_nfix) and build exe ==="
...
cd ${AD_CASE}
./case.build 2>&1 | tee build_acc_transient.log
EXEROOT=${RUNROOT}/${CASEID}_${SITE}_I1850CNRDCTCBC_ad_spinup/bld
if [ ! -f "${EXEROOT}/e3sm.exe" ]; then
  echo "ERROR: Build FAILED. Check ${AD_CASE}/build_acc_transient.log"
  exit 1
fi
...
echo "CASEID:         ${CASEID}"
echo "Job ID:         ${JID}"
echo "SourceMods:     ACC_fixed_funp_nfix"
echo "  Nfixation:    Bytnerowicz ACC — Tmin/Topt acclimate to 10-day mean soil temp (tc_soila10)"
echo "  tc < 18.5:    Tmin=2.37, Topt=30.34"
echo "  18.5-28.5:    Tmin=0.932*tc-14.87, Topt=0.574*tc+19.72"
echo "  tc >= 28.5:   Tmin=11.69, Topt=36.08"
