module ColumnDataType
  ...
  use SharedParamsMod , only : upper_soil_layer
  !
  ! !PUBLIC TYPES:
  implicit none
  save
  ...
  type, public :: column_energy_state
    ...
    real(r8), pointer :: soila10       (:)   => null() ! 10-day running mean of the 12cm soil layer temperature (K)
  ...
  contains
    ...
    procedure, public :: InitAccBuffer => init_acc_buffer_col_es
    procedure, public :: InitAccVars   => init_acc_vars_col_es
    procedure, public :: UpdateAccVars => update_acc_vars_col_es
...
contains
  ...
  subroutine col_es_init(this, begc, endc)
    ...
    allocate(this%soila10          (begc:endc))                     ; this%soila10            (:)   = nan
  ...
  subroutine init_acc_buffer_col_es (this, bounds)
    !
    ! !DESCRIPTION:
    ! Register SOIL10 accumulator (10-day running mean soil T for FUN); required
    ! so accumulRest writes SOIL10_VALUE / SOIL10_PERIOD on restart.
    !
    ! !USES
  ...
  end subroutine init_acc_buffer_col_es

  subroutine init_acc_vars_col_es (this, bounds)
    ! !DESCRIPTION:
    ...
    if (use_fun) then
       ...
       this%soila10(begc:endc) = rbufslc(begc:endc)
    end if

    deallocate(rbufslc)

  end subroutine init_acc_vars_col_es



  subroutine update_acc_vars_col_es (this, bounds)
    !
    ! USES
    use clm_time_manager, only : get_nstep
    ...
    integer :: begc, endc
    real(r8), pointer :: rbufslc(:)      ! temporary single level - patch level
    ...
    do c = begc,endc
       rbufslc(c) = this%t_soisno(c,upper_soil_layer)
    end do
    if (use_fun) then
       ...
       call extract_accum_field ('SOIL10', this%soila10, nstep)
    
    end if

    deallocate(rbufslc)
    
  end subroutine update_acc_vars_col_es
