     ...
     do fp = 1,num_soilp        ! PFT Starts
       ...
       if(ivt(p).eq.0)then
        ...
        if (istp.eq.ecm_step) then
          ...
          if(ivt(p).eq.2 .or. ivt(p).eq. 3.or. ivt(p).eq.6)then  ! ivt=2: boreal NE (added 2026-04-03)
    ...
    !                         'perECM', perecm(ivt(p))
         ...
         ! if (plant_ndemand_pool_step(p,istp) .gt. 0._r8) then   !
            ...
            do j = 1, nlevdecomp
               ...
               ! 30-day running mean soil temperature from SOILTEMP30 accumulator in TemperatureType
               tc_soila10(c)           = soila10(c)     -   tfrz
               !tc_soila10(c)          = t_soisno(c,3)  -   tfrz  ! old workaround, no longer needed
...
! Description:
!   Calculate the cost of fixing N by nodules.
!   Implements Bytnerowicz et al. (2022) temperature acclimation (MIP Option 3).
!   T* parameters acclimatise to T30 (30-day mean soil temp) via piecewise linear functions.
!   Temperate/boreal branch active here (ACC_temperate_funp_nfix — Ha1 and Bon).
!   For tropical acclimation see ACC_fixed_funp_nfix (Manaus).
! Code Description:
!   Written for E3SM by Renato Braghiere 03/16/2023
...
!--------------------------------------------------------------------------
  ...
  real(r8), intent(in) :: crootfr   ! fraction of roots for carbon that are in this layer
  real(r8), intent(in) :: s_fix     ! Inverts Houlton et al. 2008 and constrains between 7.5 and 12.5
  real(r8), intent(in) :: tc_soila10 ! 30-day running mean soil temperature, 12 cm (degrees Celsius)

  ! Temperate/boreal acclimation (MIP Option 3, section 3.3) — ACTIVE for Ha1 / Bon
  ! Protocol: same parameters for temperate AND boreal sites
  ...
  ! Tropical acclimation — commented out (use ACC_fixed_funp_nfix for Manaus)
  !if (tc_soila10 < 18.5_r8) then
