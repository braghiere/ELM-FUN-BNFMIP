...
!         sminp_vr                     => col_ps%sminp_vr                     , & ! Input:  [real(r8) (:,:) ]  (gP/m3) soil mineral P
      ...
      ! ============================================================= 
       ...
       if(.not.use_fun)then   
         ...
         ! give plants a second pass to see if there is any mineral P left over with which to satisfy residual N demand.
         do fc=1,num_soilc
            c = filter_soilc(fc)    
         ...
         do fc=1,num_soilc
           ...
           ! BUG FIX (2026-03): when use_funp=.true., P limitation is handled through
           ! FUN C costs (sminp_to_plant_new drives allocation), NOT via empirical
           ! fpg_p downscaling. Must set fpg_p=1 so GPP is not double-penalized.
            if (col_plant_pdemand(c) > 0.0_r8) then
             if(.not.use_funp)then 
               fpg_p(c) = sminp_to_plant(c) / col_plant_pdemand(c)
             else
               fpg_p(c) = 1.0_r8  ! FUN-P handles P limitation; bypass empirical scalar
             end if
            else
               fpg_p(c) = 1.0_r8
            end if
